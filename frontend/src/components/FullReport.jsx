...
import { FiCode, FiMessageSquare, FiChevronDown, FiX } from "react-icons/fi"
import {
...
import InlineChat from "./ChatBot"
...
const TOOLTIP_STYLE       = { background: "#18181b", border: "1px solid #3f3f46", borderRadius: 12, color: "#ffffff" }
const TOOLTIP_LABEL_STYLE = { color: "#a1a1aa", fontSize: 11 }


function ChartBlock({ chart_id, title, eda, currency, onAskAI }) {
  ...
  const chartMap = {
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} margin={{ top: 10, right: 20, left: 20, bottom: 40 }}>
          ...
          <Bar dataKey="margin" name="Savings Margin" radius={[4,4,0,0]}>
            {data.map((entry, i) => <Cell key={i} fill={entry.margin >= 0 ? POSITIVE : NEGATIVE} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    ),
    ...
    top_transactions: (
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={data.slice(0,10)} layout="vertical" margin={{ top: 10, right: 30, left: 200, bottom: 10 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#27272a" />
          <XAxis type="number" tick={{ fill: NEUTRAL, fontSize: 11 }} tickFormatter={(v) => `${currency}${(v/1000).toFixed(0)}k`} />
          <YAxis type="category" dataKey="narration" tick={{ fill: NEUTRAL, fontSize: 10 }} width={190} tickFormatter={(v) => v.length > 30 ? v.substring(0,30)+"..." : v} />
          <Tooltip formatter={(val) => fmt(val)} contentStyle={TOOLTIP_STYLE} labelStyle={TOOLTIP_LABEL_STYLE} />
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} margin={{ top: 10, right: 20, left: 20, bottom: 40 }}>
          ...
          <Bar dataKey="total_spending" name="Daily Spending" radius={[2,2,0,0]}>
            {data.map((entry, i) => <Cell key={i} fill={entry.is_anomaly ? "#f87171" : "#3f3f46"} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    ),
    ...
    recurring_transactions: (
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={data.slice(0,10)} layout="vertical" margin={{ top: 10, right: 30, left: 200, bottom: 10 }}>
          ...
          <XAxis type="number" tick={{ fill: NEUTRAL, fontSize: 11 }} tickFormatter={(v) => `${currency}${(v/1000).toFixed(0)}k`} />
          <YAxis type="category" dataKey="narration" tick={{ fill: NEUTRAL, fontSize: 10 }} width={190} tickFormatter={(v) => v.length > 30 ? v.substring(0,30)+"..." : v} />
          <Tooltip formatter={(val, name) => name === "count" ? val : fmt(val)} contentStyle={TOOLTIP_STYLE} labelStyle={TOOLTIP_LABEL_STYLE} />
          <Legend />
    ...
    category_breakdown: (
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          <Pie data={data.slice(0,7)} dataKey="amount" nameKey="category" cx="50%" cy="50%" outerRadius={110}
            label={({ category, percent }) => `${category} ${(percent*100).toFixed(0)}%`}
            labelLine={{ stroke: NEUTRAL }}>
            {data.slice(0,7).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} stroke="#09090b" strokeWidth={2} />)}
          </Pie>
          <Tooltip formatter={(val) => fmt(val)} contentStyle={TOOLTIP_STYLE} labelStyle={TOOLTIP_LABEL_STYLE} />
        </PieChart>
      </ResponsiveContainer>
    ...
    bank_charges_breakdown: (
      <ResponsiveContainer width="100%" height={280}>
        <PieChart>
          <Pie data={data.slice(0,6)} dataKey="amount" nameKey="category" cx="50%" cy="50%" outerRadius={100}
            label={({ category, percent }) => `${category} ${(percent*100).toFixed(0)}%`}
            labelLine={{ stroke: NEUTRAL }}>
            {data.slice(0,6).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} stroke="#09090b" strokeWidth={2} />)}
          </Pie>
          <Tooltip formatter={(val) => fmt(val)} contentStyle={TOOLTIP_STYLE} labelStyle={TOOLTIP_LABEL_STYLE} />
        </PieChart>
      </ResponsiveContainer>
  ...
  return (
    <motion.div
      ...
      className="rounded-3xl my-6 overflow-hidden"
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 pt-5 pb-2">
        ...
        <button
          onClick={() => onAskAI(chart_id, title)}
          ...
          style={{
            ...
            color: "#d08b5b",
            border: "1px solid rgba(208,139,91,0.15)",
          }}
          onMouseEnter={e => e.currentTarget.style.background = "rgba(208,139,91,0.15)"}
          onMouseLeave={e => e.currentTarget.style.background = "rgba(208,139,91,0.08)"}
        >
...
function NarrativeBlock({ block, eda, currency, onAskAI }) {
  if (block.type === "section_header") {
    return (
      <motion.h2
        ...
        className="text-2xl font-bold mt-12 mb-4 text-white pl-4"
        style={{ borderLeft: "3px solid #d08b5b" }}
      >
        {block.content}
      </motion.h2>
    )
...
export default function FullReport({ reportData, fileName }) {
  ...
  const normalizedBehavior = Array.isArray(behavior_analysis)
    ? behavior_analysis
    : behavior_analysis
      ? Object.entries(behavior_analysis).map(([metric, value]) => ({ metric, value }))
      : []

  const handleAskAI = (chartId, chartTitle) => {
    if (chatAskRef.current) {
      chatAskRef.current(chartId, chartTitle)
      // scroll to chat
      setTimeout(() => {
        document.getElementById("fiti-chat")?.scrollIntoView({ behavior: "smooth" })
      }, 100)
    }
  }
  ...
  return (
    <motion.div
      ...
      className="max-w-4xl mx-auto pb-16 text-white"
    >
      ...
      <div className="mb-10 flex items-start justify-between">
        <div>
          ...
          {fileName && (
            <p className="text-zinc-600 text-xs mt-1">{fileName}</p>
          )}
        </div>

        {/* Raw JSON toggle replaces download */}
        <button
          onClick={() => setShowRaw(!showRaw)}
          className="flex items-center gap-2 text-sm font-medium px-4 py-2.5 rounded-2xl transition mt-1"
          style={{
            background: showRaw ? "rgba(208,139,91,0.12)" : "rgba(255,255,255,0.05)",
            border: `1px solid ${showRaw ? "rgba(208,139,91,0.3)" : "rgba(255,255,255,0.1)"}`,
            color: showRaw ? "#d08b5b" : "#a1a1aa",
          }}
        >
          <FiCode size={14} />
      ...
      <div id="fiti-chat">
        <InlineChat
          ...
          behaviorAnalysis={normalizedBehavior}
          currency={currency}
          onChartQuestion={(fn) => { chatAskRef.current = fn }}
        />
      </div>
...
//         riskAnalysis={risk_analysis || {}}
//         behaviorAnalysis={
//           Array.isArray(behavior_analysis)
//             ? behavior_analysis
//             : behavior_analysis
//               ? Object.entries(behavior_analysis).map(([metric, value]) => ({ metric, value }))
//               : []
